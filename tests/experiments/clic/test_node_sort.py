...
class TestNodeSortOrder:
    ...
    def test_rejects_unknown_mode_at_construction(self):
        with pytest.raises(ValueError, match="sort_nodes_by"):
            # validated before the file check, so no file is needed to exercise it
            CLICDataset(filepath="", inputs={}, targets={"particle": []}, scale_dict_path=SCALE_DICT, sort_nodes_by="pt")
