...
## 6. Re-port onto the paper-tag trunk (2026-09-23)

Org PR #6 (Maria) takes the shared tree back to the `clic-paper` tag and rebuilds her line on it.
That tree already carries a Linformer port with: padding zeroed before the projection, q/k/v
norms applied before the zeroing, `attn_mask` refused (`ATTN_MASK_ATTN_TYPES = ["torch"]`),
value residual implemented inside the Linformer path, `set_backend` keeping weights, `share_kv`
guarded. It also has the paper-tag task heads (no double softmax, mask hit-net present,
classification at every decoder layer) and the topocluster cos/sin fix. So of the poster-era
findings, what is *not* on that tree and comes from us is:

- `data.sort_nodes_by` (reader-level; one permutation over features and incidence columns),
  with its mutation-checked tests;
- the controlled arms derived from THAT tree's `base.yaml`: `quadratic_polaris.yaml`
  (control, 12,065,291 params -- the paper model), `link32_polaris.yaml` (+64,512 = the two
  168x32 projections, decoder quadratic with mask attention ON), `link32_sortphi_polaris.yaml`;
- `tests/models/test_linformer_padding.py`: padded-vs-truncated equality without and with a
  *biased* norm (the repo LayerNorm has no bias, so a biased one is injected to make the order of
  norm and zeroing observable), garbage-in-padding invariance, norm gradients, no dead
  parameters. Mutation-checked on Maria's linformer.py: swapping zeroing before norm fails 1
  (the biased test), removing the zeroing fails 3;
- the Polaris scripts (`LOG_ROOT`, eval reads the encoder attn_type from the run config).

Recipe consequence: these arms train with **Lion 8e-5 / wd 1e-4** (the paper's), not the AdamW
recipe of the first Polaris arms (secs. 5). Numbers are not comparable across that boundary.

The k=256 both-encoder-and-decoder arm cannot be expressed on this tree (the decoder mask would
raise), which is the correct outcome of sec. 3b.
