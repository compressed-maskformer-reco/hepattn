...
3. A Python venv built from **this tree's own lock**, not a shared one. The paper-tag trunk
   pins lightning 2.5.0.post0 and its `base.yaml` and CLI use that version's Comet logger
   arguments (`project_name`, `experiment_name`, `save_dir`); lightning 2.5.2, which the
   older shared eagle venv has, rejects them at config parse. From the clone:

   ```bash
   uv python install 3.12.0 && uv sync --no-install-project --python 3.12.0
   ```

   then point `VENV_DIR` in `env.sh` at `<clone>/.venv`. (A shared venv is also only
   executable by its owner, so each person needs their own anyway.)
...
already export this; the login-node shell does not. Also note `env.sh`'s default `VENV_DIR`
(`${WORK_ROOT}/hepattn/.venv`) is executable only by its owner: anyone else must build their own
venv and set `VENV_DIR` explicitly.
